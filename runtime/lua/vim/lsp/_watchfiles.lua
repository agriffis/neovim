...
local function acquire_registration(reg, client_id, workspace_folders)
  ...
  for _, r in pairs(client_registrations) do
    if
      r.ready
      and not r.failed
      and r.watchfunc == M._watchfunc
      and r.exclude_pattern == M._poll_exclude_pattern
      and vim.deep_equal(r.options, register_options)
      and vim.deep_equal(r.workspace_folders, workspace_folders)
      and roots_unchanged(r.roots)
    then
      if client_registrations[reg.id] ~= r then
        r.refcount = r.refcount + 1
        M.unregister(reg.id, client_id)
        client_registrations[reg.id] = r
      end
      return r, true
    end
  end
...
function M.unregister(id, client_id)
  local client_registrations = registered[client_id]
  local r = client_registrations[id]
  ...
  if r then
    ...
    if r.refcount == 0 then
      for _, cancel in ipairs(r.cancels) do
        cancel()
      end
    end
  end
